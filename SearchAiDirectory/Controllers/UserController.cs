...
[Route("[action]")]
public class UserController(IHttpContextAccessor httpContextAccessor, IUserService userService) : Controller
{
    ...
    public async Task<IActionResult> Logout()
    {
        if (httpContext.User.Claims.Any())
            await userService.Logout(Convert.ToInt64(httpContext.User.Claims.First(c => c.Type == "nameid").Value));
        TempData.Clear();
