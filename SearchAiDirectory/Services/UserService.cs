...
{
    private readonly JWTokenService jwtService;
    ...
    public async Task<IList<User>> GetActiveUsers()
        => await db.Users.Include(i => i.Role).Where(w => w.IsActive).ToListAsync();

    public async Task<IList<User>> GetUsersWithTool()
        => await db.Users.Include(i => i.Role).Where(w => w.ToolID.HasValue).ToListAsync();
    ...
    public async Task<User> Login(string email, string password, bool persist)
    {
        ...
        SessionDataForAuthentication(jwtService.AuthenticateUser(user), persist);
        return user;
    }

    public async Task<bool> Logout(long userID)
    {
        await httpContext.SignOutAsync(); return true;
    }

    public async Task<User> GetUserByID(long userID)
        => await db.Users.Include(i => i.Role).SingleOrDefaultAsync(w => w.ID == userID);

    public async Task<User> GetUserByEmail(string email)
        => await db.Users.Include(i => i.Role).SingleOrDefaultAsync(w => w.Email == email);
    ...
    public async Task<string> CreateEmailCode(string email)
    {
        ...
        await db.EmailCodes.AddAsync(new()
        {
            Email = email,
            Code = code,
    ...
    public async Task<bool> ConfirmEmailCode(string email, string code)
        => await db.EmailCodes.AnyAsync(a => a.Email == email && a.Code == code);

    public async Task<bool> ChangePassword(string email, string password)
    {
        var saltpassword = await HashPassword(email, password);
        await db.Users.Where(w => w.Email.ToLower() == email.ToLower())
            .ExecuteUpdateAsync(u => u
    ...
    private async void SessionDataForAuthentication(string token, bool persist)
    {
        httpContext.Request.Headers.TryAdd("Authorization", $"Bearer {token}");
        ...
        var authProperties = new AuthenticationProperties() { RedirectUri = websiteURL, IsPersistent = persist, AllowRefresh = true };
